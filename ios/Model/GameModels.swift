...
@MainActor final class GameModel: ObservableObject {
 ...
 init(){ completed=Set(defaults.stringArray(forKey:"sequenceIQ.completed") ?? []); hints=defaults.object(forKey:"sequenceIQ.hints") as? Int ?? 3; attempts=defaults.integer(forKey:"sequenceIQ.attempts"); correctAnswers=defaults.integer(forKey:"sequenceIQ.correct"); puzzles=PuzzleRepository.load() }
 ...
 func useHint(){guard hints>0&&hintStage<3 else{return};hints-=1;hintStage+=1;persist()};func retry(){start(difficulty,level)};func next(){if level<20&&completed.contains(key(difficulty,level)){start(difficulty,level+1)}}
