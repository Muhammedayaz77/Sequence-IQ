...
class MainActivity:AppCompatActivity(){
 ...
 private val prefs by lazy{getSharedPreferences("sequenceIQ",MODE_PRIVATE)}
 override fun onCreate(b:Bundle?){super.onCreate(b);load();showSplash()}
 private fun load(){hints=prefs.getInt("hints",3);completed.addAll(prefs.getStringSet("completed",emptySet())?:emptySet())}
 private fun save(){prefs.edit().putInt("hints",hints).putStringSet("completed",completed).apply()}
 ...
 private fun canPlay(d:String,i:Int)=open(d)&&i in 1..20&&(i==1||completed.contains("$d-${i-1}"))
