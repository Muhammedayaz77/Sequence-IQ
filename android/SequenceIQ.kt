...
class MainActivity:AppCompatActivity(){
 private var level=1;private var lives=3;private var hints=3;private var difficulty="Easy";private val completed=mutableSetOf<String>();private var root:LinearLayout?=null
 ...
 private fun levels(){val v=base();v.gravity=Gravity.TOP;v.addView(btn("‹ Home"){home()});v.addView(TextView(this).apply{text="Easy Levels";textSize=25f;gravity=Gravity.CENTER});listOf("Easy","Hard","Master").forEach{d->val open=d=="Easy"||(d=="Hard"&&done("Easy")>=20)||(d=="Master"&&done("Hard")>=20);v.addView(btn(if(open)d else "🔒 $d"){if(open){difficulty=d;level=1;levels()}});if(d==difficulty&&open){for(i in 1..20){val playable=i==1||completed.contains("$d-${i-1}")||completed.contains("$d-$i");v.addView(btn(if(completed.contains("$d-$i"))"$i  ★ ★ ★" else if(playable)"$i  CURRENT" else "$i  🔒"){if(playable){level=i;lives=3;game()}}).apply{isEnabled=playable}}}}};setContentView(ScrollView(this).apply{addView(v)})}
 ...
 private fun complete(){val v=base();v.addView(TextView(this).apply{text="🏆\nLEVEL COMPLETE\n\nExcellent!\n★ ★ ★";textSize=28f;gravity=Gravity.CENTER});if(level<20)v.addView(btn("NEXT LEVEL ›"){level++;lives=3;game()});v.addView(btn("LEVEL MAP"){levels()});v.addView(btn("HOME"){home()});setContentView(v)}
