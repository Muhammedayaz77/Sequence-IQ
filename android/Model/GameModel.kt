...
class GameModel(context: Context) {
    private val prefs = context.getSharedPreferences("sequenceIQ", Context.MODE_PRIVATE)
    var screen = Screen.SPLASH
    var difficulty = Difficulty.EASY
    var level = 1
    var lives = 3
    var hints = prefs.getInt("hints", 3)
    var hintStage = 0
    var message = ""
    var attempts = prefs.getInt("attempts", 0)
    var correctAnswers = prefs.getInt("correct", 0)
    ...
    fun canPlay(d: Difficulty, l: Int) = open(d) && l in 1..20 && (l == 1 || completed.contains("${d.title}-$l".replace("$l", "${l-1}")))
    fun completed(d: Difficulty, l: Int) = completed.contains("${d.title}-$l")
    fun completedCount(d: Difficulty) = completed.count { it.startsWith("${d.title}-") }
    fun puzzle(): Puzzle { val a=level+1; return when(difficulty) { Difficulty.EASY -> Puzzle(difficulty,level,listOf(a,a+2,a+4,a+6,null,a+10),a+8,listOf(a+8,a+12,a+4,a+14)); Difficulty.HARD -> Puzzle(difficulty,level,listOf(a,a*2,a*4,a*8,null,a*32),a*16,listOf(a*16,a*12,a*24,a*32)); Difficulty.MASTER -> Puzzle(difficulty,level,listOf(a,a+3,a+8,a+15,null,a+35),a+24,listOf(a+24,a+31,a+21,a+36)) } }
    ...
    fun useHint(){if(hints>0&&hintStage<3){hints--;hintStage++;persist()}}
    fun retry(){start(difficulty,level)}
    fun next(){if(level<20&&completed(difficulty,level))start(difficulty,level+1)}
